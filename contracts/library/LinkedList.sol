...
library MinLinkedListLib {
    ...
    /// if the price of order id0 is less than the price of order id1
    function compare(
    ...
    function insert(
        ...
        hintId = findIndexToInsert(listData, orderId, hintId);

        uint32 next = listData.list[hintId].next;
        listData.list[orderId] = Node({prev: hintId, next: next, active: true});
        listData.list[hintId].next = orderId;
        listData.list[next].prev = orderId;
    }

    /// @notice Update the position of an order id in the linked list to
    ...
    function update(
        ...
        hintId = findIndexToInsert(listData, orderId, hintId);
        uint32 newNext = listData.list[hintId].next;

        listData.list[orderId].prev = hintId;
        listData.list[orderId].next = newNext;
        listData.list[hintId].next = orderId;
        listData.list[newNext].prev = orderId;
    }
    ...
    /// list is sorted, this gets the order id with the lowest price
...
library MaxLinkedListLib {
    /// @notice Comparison function for linked list. Returns true
    /// if the price of order id0 is greater than the price of order id1
    function compare(
        ...
        return
            FullMath.mulCompare(
                listData.idToLimitOrder[id_1].amount1,
                listData.idToLimitOrder[id_0].amount0,
                listData.idToLimitOrder[id_1].amount0,
                listData.idToLimitOrder[id_0].amount1
            );
    }
    ...
    /// @notice Get the first order id in the linked list. Since the linked
    /// list is sorted, this gets the order id with the highest price
    /// @dev Order id 0 is a dummy value and should not be returned
